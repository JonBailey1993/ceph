                                                  ...
                                                  stringstream& errstream)
{
  ...
  if (auth_version != eversion_t() && m_is_optimized_ec &&
    ...
    if (m_pg.ec_can_decode(available_shards)) {
      ...
      bufferlist crc_bl;
      std::map<uint64_t, unsigned int> length_zero_data_hash_map;
      for (shard_id_t shard_id; shard_id.id < m_pg.get_ec_data_chunk_count();
           ++shard_id) {
        if (!length_zero_data_hash_map.contains(ret_auth.auth_oi.size))
        {
          ...
          zero_bl.append(buffer::create(
              logical_to_ondisk_size(ret_auth.auth_oi.size, shard_id), 0));

          bufferhash zero_data_hash(-1);
          zero_data_hash << zero_bl;
          unsigned int zero_data_crc = zero_data_hash.digest();

          length_zero_data_hash_map.insert({ret_auth.auth_oi.size, zero_data_crc});
        }

        unsigned int zero_data_crc = length_zero_data_hash_map.at(ret_auth.auth_oi.size);
  ...
  if (!m_is_replicated && m_is_optimized_ec)
  {
    ...
    if (std::any_of(digests.begin(), digests.end(),
          [](const std::pair<const shard_id_t, ceph::bufferlist&>& digest)
            { return digest.second != NULL && !std::string(digest.second.c_str()).empty(); }))
    {
      // Unseed all buffers in chunks
      for (auto& [srd, bl] : digests)
      {
        if (!length_zero_data_hash_map.contains(auth_sel.auth_oi.size))
        {
          // Shards can have different lengths.
          // Lengths of zero buffers need to match the length of the shard.
          // So we initialise a new buffer to the correct length per shard.
          ...
          length_zero_data_hash_map.insert({auth_sel.auth_oi.size, zero_data_crc});
        }

        unsigned int zero_data_crc = length_zero_data_hash_map.at(auth_sel.auth_oi.size);
