...
class TestPg : public PgScrubBeListener {
 ...
 public:
                                                ...
                                                int chunk_size) const final
  {
    ...
    {
      bufferlist bl;
      bl.append(buffer::create(chunk_size, 0));
      bl.rebuild();
    ...
    for (shard_id_t i{0}; i < get_ec_data_chunk_count(); ++i)
    {
      ...
      {
        encode_map.at(i).c_str()[j] = chunks[j + (chunk_size * i.id)];
        for (shard_id_t k{static_cast<int8_t>(get_ec_stripe_width())}; k < get_ec_stripe_width(); ++k)
        {
          encode_map.at(k).c_str()[j] += chunks[j + (chunk_size * i.id)];
        }
      }
    }
  ...
  shard_id_map<bufferlist> ec_decode_acting_set(
    ...
    for (shard_id_t shard{0}; shard < get_ec_stripe_width(); ++shard)
    {
      if (!chunks.contains(shard))
      {
        for (int j = 0; j < chunk_size; j++)
        {
          if (shard.id < get_ec_data_chunk_count())
          {
            decode_map.at(shard).c_str()[j] = decode_map.at(shard_id_t{static_cast<int8_t>(get_ec_data_chunk_count())}).c_str()[j];
          }
          for (shard_id_t i{0}; i < get_ec_data_chunk_count(); ++i)
          {
            if (shard.id < get_ec_data_chunk_count() && chunks.contains(i))
            {
...
void TestTScrubberBe::SetUp()
{
  ...
  real_objs_list =
    ScrubGenerator::make_real_objs_conf(pool_id, real_objs, acting_osds, acting_shards, pool_conf.erasure_code_profile.has_value());
				     ...
				     const ScrubGenerator::pool_conf_t& pconf)
{
  ...
  {
    osdmap->set_erasure_code_profile(pconf.erasure_code_profile->erasure_code_profile_name, pconf.erasure_code_profile->erasure_code_profile);
...
public:
  ...
  int8_t k = 4;
  int8_t m = 2;
  int m_chunk_size = 4; // Static - We don't need more than this
...
public:
  ...
  TestTScrubberBeParams inject_params() override
  {
    ...
    params.objs_conf.objs[(rng() % k)].corrupt_funcs = &ScrubDatasets::crpt_funcs_set2;
    return params;
  }
};
...
public:
  ...
  TestTScrubberBeParams inject_params() override
  {
    ...
    {
      params.objs_conf.objs[shard_ids.at(i)].corrupt_funcs = &ScrubDatasets::crpt_funcs_set2;
    }
    return params;
  }
};
...
public:
  ...
  {
    TestTScrubberBeParams params = TestTScrubberBeECCorruptShards::inject_params();
    params.objs_conf.objs[k].corrupt_funcs = &ScrubDatasets::crpt_funcs_set2;
    return params;
  }
};
