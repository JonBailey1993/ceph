...
struct MappingLayerPair {};
void validate(boost::any& v, const std::vector<std::string>& values,
              MappingLayerPair *target_type, int) {
  po::validators::check_first_occurrence(v);
  const std::string &s = po::validators::get_single_string(values);
  auto part = ceph::split(s, ":").begin();
  std::string parse_error;
  std::string_view first = *part++;
  std::string_view second = *part;
  if (!parse_error.empty()) {
    throw po::validation_error(po::validation_error::invalid_option_value);
  }
  v = boost::any(std::pair<std::string,std::string>{first,second});
}
...
po::options_description get_options_description()
{
  ...
  desc.add_options()
    ...
    ("technique", po::value<std::string>(),
      "EC profile technique")
    ("packetsize", po::value<uint64_t>(),
      "EC profile packetsize")
    ("c", po::value<uint64_t>(),
      "EC profile c value")
    ("w", po::value<uint64_t>(),
      "EC profile w value")
    ("mappinglayers", po::value<MappingLayerPair>(),
      "Colon seperated EC profile mapping and layers values")
...
ceph::io_sequence::tester::SelectSeqRange::SelectSeqRange(po::variables_map& vm)
    : ProgramOptionReader<std::pair<ceph::io_exerciser::Sequence, ceph::io_exerciser::Sequence>>(vm, "sequence") {
...
const std::pair<ceph::io_exerciser::Sequence,ceph::io_exerciser::Sequence>
  ceph::io_sequence::tester::SelectSeqRange
    ::select()
{
...
ceph::io_sequence::tester::SelectErasureTechnique
...
const std::vector<std::string> ceph::io_sequence::tester::SelectErasureTechnique::generate_selections()
{
...
ceph::io_sequence::tester::SelectErasureKM
...
const std::vector<std::pair<int,int>> ceph::io_sequence::tester::SelectErasureKM
  ::generate_selections()
{
  ...
  else if (plugin == "shec" || (plugin == "jerasure" && (technique == "liberation" ||
                                    technique == "blaum_roth")))
  {
    for (int m = 1; m <= 2; m++)
      for (int k = 2; k <= 6; k++)
        selection.push_back({k, m});
  } else if (plugin == "jerasure" && (technique == "reed_sol_r6_op" ||
...
ceph::io_sequence::tester::SelectErasurePacketSize
...
const std::vector<uint64_t> ceph::io_sequence::tester::SelectErasurePacketSize
  ::generate_selections()
{
  ...
  if (km && technique &&
      technique == "liberation" && km->first == 6 && km->second == 2)
  {
    selection.push_back(32);
  ...
  if (km && technique &&
      technique == "blaum_roth" && km->first == 6 && km->second == 2)
  {
  ...
  if (km && technique &&
      technique == "liber8tion" && km->first == 6 && km->second == 2)
  {
...
ceph::io_sequence::tester::SelectErasureC
...
const std::vector<uint64_t> ceph::io_sequence::tester::SelectErasureC::generate_selections()
{
  if (plugin != "shec")
  {
    return {};
...
ceph::io_sequence::tester::SelectErasureW
...
const std::vector<uint64_t> ceph::io_sequence::tester::SelectErasureW::generate_selections()
{
  std::vector<uint64_t> selection = {};

  if (technique && km &&
      technique == "reed_sol_van" && km->first == 6 && km->second == 3)
  {
...
ceph::io_sequence::tester::SelectErasureChunkSize
...
const std::vector<uint64_t> ceph::io_sequence::tester::SelectErasureChunkSize::generate_selections()
{
  int minimum_granularity = ec_impl->get_minimum_granularity();

  std::vector<uint64_t> choices = {};

  if (4096 % minimum_granularity == 0)
  {
    choices.push_back(4096);
  }
  else
  {
    choices.push_back(minimum_granularity * rng(4));
  }

  if ((64*1024) % minimum_granularity == 0)
  {
    choices.push_back(64*1024);
  }
  else
  {
    choices.push_back(minimum_granularity * rng(64));
  }

  if ((256*1024) % minimum_granularity == 0)
  {
    choices.push_back(256*1024);
  }
  else
  {
    choices.push_back(minimum_granularity * rng(256));
  }
...
ceph::io_sequence::tester::SelectErasureProfile
  ...
  sml{ rng, vm, "mappinglayer", true }
{
  if (isForced())
  {
    std::array<std::string, 9> disallowed_options =
    {
      "pool", "km", "technique", "packetsize", "c", "w", "mappinglayers",
      "chunksize"
    };
    ...
    for (std::string& option : disallowed_options)
    {
      ...
      {
        ceph_abort_msg(fmt::format("{} option not allowed "
                                   "if profile is specified", option));
      }
    }
  }
}

const ceph::io_sequence::tester::Profile ceph::io_sequence::tester
  ::SelectErasureProfile::select()
{
  ...
  else
  {
    ...
    SelectErasurePacketSize sps { rng, vm, profile.plugin, profile.technique,
                                  profile.km, first_use };
    profile.packet_size = sps.select();

    SelectErasureW ssw { rng, vm, profile.plugin, profile.technique,
                          profile.km, profile.packet_size, first_use };
    profile.w = ssw.select();

    SelectErasureC ssc { rng, vm, profile.plugin, profile.km, first_use };
    profile.c = ssc.select();

    if (profile.plugin == "lrc")
    {
      std::pair<std::string_view, std::string_view> mappingAndLayers = sml.select();
    ...
    {
      erasureCodeProfile["jerasure_per_chunk_alignment"] = std::to_string(*profile.jerasure_per_chunk_alignment);
    }
    if (profile.mapping)
    {
    ...
    // ErasureCodeInterfaceRef ec_impl;
    // std::stringstream ss;
    // instance.factory(std::string(profile.plugin), cct->_conf.get_val<std::string>("erasure_code_dir"), erasureCodeProfile, &ec_impl, &ss);
    // ceph_assert(ec_impl);

    // if (profile.plugin != "clay")
    // {
    ...
    if (profile.km)
    {
      profile.name += fmt::format("_k{}_m{}", profile.km->first, profile.km->second);
    }
    if (profile.packet_size)
    {
...
void ceph::io_sequence::tester::SelectErasureProfile
  ::create(ceph::io_sequence::tester::Profile profile)
{
  ...
  ceph::messaging::osd::OSDECProfileSetRequest ecProfileSetRequest{ profile.name, profile_values };
  int rc = send_mon_command(ecProfileSetRequest, rados, "OSDECProfileSetRequest",
                        inbl, &outbl, formatter.get());
  ceph_assert(rc == 0);
}

const ceph::io_sequence::tester::Profile ceph::io_sequence::tester
  ::SelectErasureProfile::selectExistingProfile(const std::string& profile_name)
{
  int rc;
  bufferlist inbl, outbl;
  auto formatter = std::make_shared<JSONFormatter>(false);

  ceph::messaging::osd::OSDECProfileGetRequest osdECProfileGetRequest{ profile_name };
...
ceph::io_sequence::tester::SelectErasurePool
  ...
  sep{ cct, rng, vm, rados, dry_run, first_use }
{
  if (isForced())
  {
    std::array<std::string, 9> disallowed_options =
    {
      "profile", "km", "technique", "packetsize", "c", "w", "mappinglayers",
      "chunksize"
    };
...
const std::string ceph::io_sequence::tester::SelectErasurePool::select()
{
  ...
  if (!dry_run)
  {
    ...
    if (!dry_run)
    {
      configureServices(allow_pool_autoscaling,
                        allow_pool_balancer,
                        allow_pool_deep_scrubbing,
                        allow_pool_scrubbing,
                        test_recovery);
...
std::string ceph::io_sequence::tester::SelectErasurePool::create()
{
  ...
  ceph::messaging::osd::OSDECPoolCreateRequest poolCreateRequest{pool_name, "erasure", 8, 8, profile->name};
...
void ceph::io_sequence::tester::SelectErasurePool
  ::configureServices(bool allow_pool_autoscaling,
                      bool allow_pool_balancer,
                      bool allow_pool_deep_scrubbing,
                      bool allow_pool_scrubbing,
                      bool test_recovery)
{
  ...
  {
      ceph::messaging::osd::OSDSetRequest setNoAutoscaleRequest{ "noautoscale", std::nullopt };
      rc = send_mon_command(setNoAutoscaleRequest, rados, "OSDSetRequest",
                        inbl, &outbl, formatter.get());
      ceph_assert(rc == 0);
  }
  ...
  if (!allow_pool_balancer)
  {
    ...
    rc = send_mon_command(balancerOffRequest, rados, "BalancerOffRequest",
                        inbl, &outbl, formatter.get());
    ceph_assert(rc == 0);
  ...
  if (allow_pool_deep_scrubbing) {
    ceph::messaging::osd::OSDSetRequest setNoDeepScrubRequest{"nodeep-scrub", std::nullopt};
...
ceph::io_sequence::tester::TestRunner::TestRunner(boost::intrusive_ptr<CephContext> cct,
                                                  po::variables_map& vm,
                                                  librados::Rados& rados)
    : rados(rados),
...
void ceph::io_sequence::tester::TestRunner::list_sequence(bool testrecovery) {
  ...
  if (testrecovery)
  {
    ...
    if (profile)
    {
      ...
      {
        mappingLayers = { *spo.getProfile()->mapping,
                              *spo.getProfile()->layers };
      }
    }
...
int main(int argc, char** argv) {
  ...
  try {
    runner = std::make_unique<ceph::io_sequence::tester::TestRunner>(cct, vm, rados);
  } catch(const po::error& e) {
    return 1;
  }
  runner->run_test();
