...
TEST_P(PluginTest,CRCEncodeDecodeSupport)
{
  ...
  for (;it != encoded.end();)
  {
    ...
    // ************************************************************************************************************************************************************************
    // CHANGE TO get_k_plus_m() TO TEST AGAINST MORE VALUES OF M
    // ************************************************************************************************************************************************************************
    if (shard_id < get_k() + 1)
    {
      ...
      // Compare Encoded data CRCs and Encoded CRCs and assert equal
      // This technique only works for the first parity shard, so that is all
      // we test
    ...
    // {
      ...
      // ************************************************************************************************************************************************************************
      // UNCOMMENT TO EXPLICITLY VERIFY CRCS ARE NOT VALID FOR M=2 OR HIGHER
      // ************************************************************************************************************************************************************************
      // std::cout << std::hex << std::setw(8) << crc << " != " << crc2 << std::endl;
      // EXPECT_NE(crc,crc2);
    // }
  ...
  if (erasure_code->get_supported_optimizations() &
      ErasureCodeInterface::FLAG_EC_PLUGIN_ENCODE_CRCS_TO_PARITY_CRCS) {
    // Plugin should not have FLAG_EC_PLUGIN_ENCODE_CRCS_TO_PARITY_CRCS enabled, this
    // failure proves that it can cause a data integrity issue
    EXPECT_EQ(different, false);
    }
}
